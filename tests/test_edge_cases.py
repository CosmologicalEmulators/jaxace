...
class TestNaNInfPropagation:
    ...
    def test_E_z_with_nan_inputs(self):
        ...
        result = E_z(jnp.nan, 0.3, 0.67)
        assert jnp.isnan(result), "E_z should propagate NaN in redshift"
        
        # Test with NaN in Ωcb0
        result = E_z(1.0, jnp.nan, 0.67)
        assert jnp.isnan(result), "E_z should propagate NaN in Ωcb0"
        
        # Test with NaN in h
        result = E_z(1.0, 0.3, jnp.nan)
        assert jnp.isnan(result), "E_z should propagate NaN in h"
        
        # Test with NaN in optional parameters
        result = E_z(1.0, 0.3, 0.67, mν=jnp.nan)
        assert jnp.isnan(result), "E_z should propagate NaN in neutrino mass"
        
        result = E_z(1.0, 0.3, 0.67, w0=jnp.nan)
...
class TestEmptyArrays:
    ...
    def test_distance_functions_with_empty_arrays(self):
        ...
        # Test dA_z
        result = dA_z(z_empty, 0.3, 0.67)
        assert result.shape == (0,), "dA_z should return empty array"
        
        # Test dL_z
        result = dL_z(z_empty, 0.3, 0.67)
    ...
    def test_concatenate_with_empty(self):
        ...
        result = E_z(z_combined, 0.3, 0.67)
...
class TestMixedPrecision:
    ...
    def test_mixed_precision_inputs(self):
        ...
        # Mix float32 and float64 inputs
        z_64 = jnp.array(1.0, dtype=jnp.float64)
        Ωcb0_32 = jnp.array(0.3, dtype=jnp.float32)
        h_64 = jnp.array(0.67, dtype=jnp.float64)
        
        # JAX should promote to highest precision
        result = E_z(z_64, Ωcb0_32, h_64)
    ...
    def test_array_precision_preservation(self):
        ...
        result_64 = E_z(z_array_64, jnp.float64(0.3), jnp.float64(0.67))
        ...
        result_32_promoted = E_z(z_array_32, jnp.float64(0.3), jnp.float64(0.67))
...
class TestBoundaryConditions:
    ...
    def test_zero_redshift(self):
        ...
        assert jnp.isclose(E_z(z, Ωcb0, h), 1.0), "E(z=0) should be 1"
        
        # Distances at z=0 should be exactly 0
        assert jnp.isclose(r_z(z, Ωcb0, h), 0.0), "r(z=0) should be 0"
        assert jnp.isclose(dA_z(z, Ωcb0, h), 0.0), "dA(z=0) should be 0"
        assert jnp.isclose(dL_z(z, Ωcb0, h), 0.0), "dL(z=0) should be 0"
    ...
    def test_extreme_parameters(self):
        ...
        # Very small Ωcb0 (dark energy dominated)
        result = E_z(1.0, 0.001, 0.67)
        assert jnp.isfinite(result), "Should handle very small Ωcb0"
        
        # Very large Ωcb0 (matter dominated)
        result = E_z(1.0, 0.999, 0.67)
        assert jnp.isfinite(result), "Should handle very large Ωcb0"
        
        # Extreme w0 values
        result = E_z(1.0, 0.3, 0.67, w0=-2.0)
        assert jnp.isfinite(result), "Should handle w0 < -1"
        
        result = E_z(1.0, 0.3, 0.67, w0=0.0)
