...
class TestArtifactLoading:
    ...
    def test_artifacts_toml_exists(self):
        """Test that Artifacts.toml exists in package directory."""
        package_dir = Path(jaxace.__file__).parent
        artifacts_toml = package_dir / "Artifacts.toml"
        assert artifacts_toml.exists(), "Artifacts.toml should exist in package directory"
...
# =============================================================================
# ln10As Basis Emulator Tests
# =============================================================================


class TestLn10AsArtifactLoading:
    """Test loading ln10As basis emulator from artifacts."""

    def test_load_ln10As_emulator(self):
        """Test loading ACE_mnuw0wacdm_ln10As_basis emulator from artifact."""
        emu = jaxace.load_trained_emulator_from_artifact('ACE_mnuw0wacdm_ln10As_basis')
    ...
    def test_ln10As_emulator_dimensions(self):
        """Test that ln10As emulator has correct input/output dimensions."""
        emu = jaxace.load_trained_emulator_from_artifact('ACE_mnuw0wacdm_ln10As_basis')

        # Should have 9 inputs and 7 outputs (same structure as sigma8)
        assert emu.in_minmax.shape == (9, 2), f"Expected 9 input parameters, got {emu.in_minmax.shape[0]}"
        assert emu.out_minmax.shape == (7, 2), f"Expected 7 output quantities, got {emu.out_minmax.shape[0]}"


class TestLn10AsEmulatorEvaluation:
    """Test ln10As emulator evaluation with specific inputs."""

    @pytest.fixture
    def emulator(self):
        """Load ln10As emulator for testing."""
        return jaxace.load_trained_emulator_from_artifact('ACE_mnuw0wacdm_ln10As_basis')

    @pytest.fixture
    def test_input(self):
        """
        Test input parameters for ln10As basis.

        Parameters: z, ln10As, ns, H0, ombh2, omch2, Mν, w0, wa
        """
        return np.array([
            0.7765,                      # z
            3.044,                       # ln10As (instead of sigma8)
            1.05795,                     # ns
            81.84666666666666,           # H0
            0.020955833333333333,        # ombh2
            0.08851666666666666,         # omch2
    ...
    def test_output_is_finite(self, emulator, test_input):
        ...
        # Individual checks for debugging
        output_names = ['sigma8', 'sigma8_z', 'r_drag', 'H_z', 'r_z', 'D_z', 'f_z']
        for i, (name, value) in enumerate(zip(output_names, output)):
            assert jnp.isfinite(value), (
                f"Output {i} ({name}) is not finite: {value}"
            )
    ...
    def test_output_values_reasonable(self, emulator, test_input):
        ...
        # sigma8 should be around 0.8
        assert 0.5 < output[0] < 1.2, f"sigma8 = {output[0]} out of range"

        # sigma8_z should be positive and less than 1.5
        assert 0.0 < output[1] < 1.5, f"sigma8_z = {output[1]} out of range"
...
class TestLn10AsJAXAutodiff:
    """Test JAX autodiff compatibility for ln10As emulator."""
