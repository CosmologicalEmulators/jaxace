...
def _get_weight_bias(i: int, n_in: int, n_out: int, 
    ...
    Matches Julia's _get_weight_bias function.
    """
    # Reshape weights (note: Julia uses column-major, Python uses row-major)
    # In Julia: reshape(weight_bias[i:i+n_out*n_in-1], n_out, n_in)
    # For Flax, we need (n_in, n_out) shape
    weight = np.reshape(weight_bias[i:i+n_out*n_in], (n_in, n_out), order='F')
